// WordPress Compatible Image Compressor JavaScript - Use with WP Code Inserter
// All IDs are prefixed with 'wp-' to avoid conflicts with WordPress
...
(function() {
  ...
  ready(function() {
    ...
    const container = document.querySelector('.wp-image-compressor-container');
    if (!container) {
      console.log('WordPress Image Compressor: Container not found');
      return;
    }

    // Get all elements with prefixed IDs
    ...
    const handle = dragDivider.querySelector('.wp-image-compressor-handle');
    ...
    function renderList() {
      ...
      images.forEach(item => {
        ...
        const card = node.querySelector('.wp-image-compressor-image-card');
        const thumb = node.querySelector('.wp-image-compressor-thumb');
        const name = node.querySelector('.wp-image-compressor-name');
        const size = node.querySelector('.wp-image-compressor-size');
        const type = node.querySelector('.wp-image-compressor-type');
        const reduction = node.querySelector('.wp-image-compressor-reduction');
        const remove = node.querySelector('.wp-image-compressor-remove');
        ...
        remove.addEventListener('click', () => {
          ...
          modalDesc.textContent = `Are you sure you want to delete "${item.name}"?`;
        ...
        card.addEventListener('click', () => {
          ...
          const prev = imageListEl.querySelector('.wp-image-compressor-image-card.selected');
    ...
    if (fileInput) {
      fileInput.addEventListener('change', async (e) => {
        ...
        if (images.length === 1) {
          ...
          const firstCard = imageListEl.querySelector('.wp-image-compressor-image-card');
    ...
    if (deleteAllBtn) {
      deleteAllBtn.addEventListener('click', () => {
        ...
        modalDesc.textContent = `Are you sure you want to delete all ${images.length} images?`;
        confirmModal.showModal();
      });
    }
    ...
    if (filterMenu) {
      filterMenu.addEventListener('click', (e) => {
        if (!e.target.matches('.wp-image-compressor-menu-item')) return;
    ...
    if (resetToolBtn) {
      resetToolBtn.addEventListener('click', () => {
        ...
        modalDesc.textContent = 'Are you sure you want to reset? This will clear all images and settings.';
        confirmModal.showModal();
      });
    }
    ...
    if (uploadFrame) {
      uploadFrame.addEventListener('dragover', (e) => {
        e.preventDefault();
        uploadFrame.classList.add('wp-image-compressor-drag-over');
      });

      uploadFrame.addEventListener('dragleave', () => {
        uploadFrame.classList.remove('wp-image-compressor-drag-over');
      });
    ...
    if (imageListEl) {
      imageListEl.addEventListener('click', async (e) => {
        ...
        const card = actionBtn.closest('.wp-image-compressor-image-card');
        if (!card) return;
    ...
    console.log('WordPress Image Compressor: Initialized successfully');
  });
})();