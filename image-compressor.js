(() => {
  ...
  async function compressFile(file, qualityPercent) {
    ...
    const canvas = document.createElement('canvas'); canvas.width = imgBitmap.width; canvas.height = imgBitmap.height; const ctx = canvas.getContext('2d'); ctx.clearRect(0,0,canvas.width,canvas.height); ctx.drawImage(imgBitmap, 0, 0);
    const isPng = file.type.includes('png'); const mimeType = isPng ? 'image/png' : 'image/jpeg'; const q = isPng ? 1 : Math.min(Math.max(qualityPercent / 100, 0.01), 1);
    const blob = await new Promise(resolve => canvas.toBlob(resolve, mimeType, q)); return blob || file;
  }
  ...
  function addFiles(fileList) { if (!fileList || !fileList.length) return; showProcessing('Uploading...'); const files = Array.from(fileList).filter(f => f && f.type.startsWith('image/')); for (const file of files) { const id = generateId(); const url = URL.createObjectURL(file); images.push({ id, file, url, originalBytes: file.size, name: file.name, type: file.type }); } hideProcessing(); setEmptyAndRender(); }

  function removeImage(id) { const idx = images.findIndex(i => i.id === id); if (idx === -1) return; const item = images[idx]; URL.revokeObjectURL(item.url); if (item.cachedCompressed) URL.revokeObjectURL(item.cachedCompressed.url); images.splice(idx, 1); setEmptyAndRender(); }
  ...
  // Max size apply via binary search for JPEG
